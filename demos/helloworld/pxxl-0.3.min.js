...
function not(p){var p=toParser(p);var pid=parser_id++;return function(state){var savedState=state;var cached=savedState.getCached(pid);if(cached)return cached;cached=p(state)?false:make_result(state,"",undefined);savedState.putCached(pid,cached);return cached}};var Pxxl={};Pxxl.Font=function(version,comments,properties,glyphs){this.version=version;this.comments=comments;this.properties=properties;this.glyphs=glyphs};Pxxl.Font.ParseJSON=function(obj){var f=new Pxxl.Font(obj.version,obj.comments,obj.properties,{});for(var k in obj)if(obj.hasOwnProperty(k)&&k!="glyphs")f[k]=obj[k];f.glyphs={};for(var g in obj.glyphs)if(obj.glyphs.hasOwnProperty(g))f.glyphs[g]=Font.Glyph.ParseJSON(obj.glyphs[g]);return f};
Pxxl.Font.prototype={size:function(){return this.SIZE[0]},getGlyph:function(character){var c=character.charCodeAt(0);return this.glyphs[c]},defaultWidth:function(){return this.FONTBOUNDINGBOX[0]},defaultHeight:function(){return this.FONTBOUNDINGBOX[1]},bit:function(text,row,column){var t=~~(column/8);if(t<0||t>text.length-1)return false;var c=text.charCodeAt(t);var g=this.glyphs[c];if(g)return g.bit(row,column%8);else return false},getPixels:function(text){var ctx=this.ctx;var hspacing=this.FONTBOUNDINGBOX[0];
var pixels=[];for(var t=0;t<text.length;t++){var chr=text.charCodeAt(t);var glyph=this.glyphs[chr];var bitmap=glyph.bitmap;var dx=t*hspacing;var dy=this.defaultHeight()-glyph.height();for(var r=0;r<bitmap.length;r++){var row=bitmap[r];for(var b=0;b<row.length;b++){var byt=row[b];var offset=b*8;var bit=256;while(bit>>>=1){if(byt&bit){var px=dx+offset;var py=dy+r;pixels.push({x:px,y:py,row:r,column:offset})}offset++}}}}return pixels}};Pxxl.Glyph=function(name,bitmap){this.name=name;this.bitmap=bitmap};Pxxl.Glyph.ParseJSON=function(obj){var g=new Font.Glyph(obj.name,obj.bitmap);for(var k in obj)if(obj.hasOwnProperty(k))g[k]=obj[k];return g};
Pxxl.Glyph.prototype={set:function(x,y,value){var bit=1<<this.width()-x-1;var byt=~~(bit/256);bit%=(byt+1)*256;if(value)this.bitmap[y][byt]|=bit;else this.bitmap[y][byt]&=~bit},get:function(x,y){var bit=1<<this.width()-x-1;var byt=~~(bit/256);bit%=(byt+1)*256;var result=this.bitmap[y][byt]&bit;return!!result},width:function(){return this.BBX[0]},height:function(){return this.BBX[1]},toString:function(){var result="";for(var y=0;y<this.bitmap.length;y++){for(var x=0;x<this.width();x++)result+=this.get(x,
y)?"*":" ";result+="/n"}return result}};(function(){var LoadFont=function LoadFont(url,callback){if(url.indexOf("json")>-1)$.getJSON(url,function(data){callback(Pxxl.Font.ParseJSON(data))});else $.get(url,function(data){callback(Pxxl.Font.ParseBDF(data))},"text")};function memoize2(f){var cache={};return function(arg,callback){var cached=cache[arg];if(typeof cached!=="undefined")return callback(cached);else return f(arg,function(result){cache[arg]=result;return callback(result)})}}Pxxl.LoadFont=memoize2(LoadFont)})();(function(){var EXCLAMATION_MARK=ch("!");var AT=ch("@");var HASH=ch("#");var DOLLAR=ch("$");var PERCENT=ch("%");var CARET=ch("^");var AMPERSAND=ch("&");var ASTERISK=ch("*");var LEFT_PARENTHESIS=ch("(");var RIGHT_PARENTHESIS=ch(")");var MINUS=ch("-");var UNDERSCORE=ch("_");var PLUS=ch("+");var EQUALS=ch("=");var LEFT_ACCOLADE=ch("{");var RIGHT_ACCOLADE=ch("}");var LEFT_BRACKET=ch("[");var RIGHT_BRACKET=ch("]");var COLON=ch(":");var SEMICOLON=ch(";");var QUOTE=ch("'");var DOUBLE_QUOTE=ch('"');var PIPE=
...
pick(2,sequence(COMMENT,optional(Space),optional(Text)));var CommentRow=trace(pick(0,sequence(Comment,EOL)),"comment");var BDF=action(sequence(repeat0(CommentRow),FontStart,repeat0(CommentRow),repeat0(butnot(PropRow,GlyphStart)),repeat0(Glyph),FontEnd),MakeFont);function MakeFont(ast){var formatVersion=ast[1];var comments=ast[0].concat(ast[2]);var properties=ast[3];var glyphs=PropertyList2Hash(ast[4]);var f=new Pxxl.Font(formatVersion,comments,properties,glyphs);return PropertyBagMixin(f,properties)}
function MakeGlyph(ast){var name=ast[0];var properties=ast[1];var bitmap=ast[2];var g=new Pxxl.Glyph(name,bitmap);g=PropertyBagMixin(g,properties);return{name:g["ENCODING"],value:g}}function PropertyBagMixin(obj,proplist){for(var i=0;i<proplist.length;i++){var prop=proplist[i];obj[prop.name]=prop.value}return obj}function PropertyList2Hash(proplist){var hash={};for(var i=0;i<proplist.length;i++){var prop=proplist[i];hash[prop.name]=prop.value}return hash}function MakeProp1(ast){var value=ast[1];var name=
ast[0];if(name=="ENCODING"||name=="CHARS")value=value[0];return{name:name,value:value}}function MakeProp2(ast){return{name:ast[0],value:ast[2]}}function flatten(p){return join_action(p,"")}function pick(i,p){return action(p,function(ast){return ast[i]})}function trace(p,label){var traceon=Pxxl.trace;var traceall=Pxxl.traceall;if(!traceon)return p;return function(state){var result=p(state);if(!result.ast){var matched=state.input.substring(0,state.index);var lines=matched.split("\n");console.error(label,
"failed at line",lines.length,state)}if(result.ast&&traceall)console.log(label,"matches",result.matched,"\nAST:",result.ast);return result}}function pre(input){var lines=input.split("\n");for(var l=lines.length-1;l>=0;l--){var line=ltrim(lines[l]);if(line=="")lines.splice(l,1);else lines[l]=line}return lines.join("\n")}function ltrim(stringToTrim){return stringToTrim.replace(/^\s+/,"")}Pxxl.Font.ParseBDF=function(input,trace,traceall){Pxxl.trace=trace;Pxxl.traceall=traceall;input=pre(input);var state=
ps(input);var before=+new Date;var result=BDF(state);var time=+new Date-before;if(result.ast)return result.ast;throw new Error("Unable to parse font!");}})();function pxxl(fontUrl,text,draw){Pxxl.LoadFont(fontUrl,function(font){var pixels=font.getPixels(text);draw(pixels)})};
